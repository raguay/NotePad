...
class $$NoteObject extends Record {}

$$NoteObject.mappings = {"note":null}

$$NoteObject.decode = (_input) => {
  let note = Decoder.field(`note`, Decoder.string)(_input)
  if (note instanceof Err) { return note }

  return new Ok(new $$NoteObject({
    note: note.value
  }))
}
...
_program.addRoutes([{
  ...
  try {
    _result = await $Notes_Store.loadNotes()
  }
  catch (_error) {
...
const $Notes_Store = new (class extends Store {
    constructor() {
    super()
    this.state = {
  ...
  setNote(id, text) {
    return (async () => {
      ...
      try {
        ...
        this.setState(_update(this.state, new Record({
      notes: $Array.mapWithIndex(((oldText, index) => {
      return (_compare(id, index) ? text : oldText)
      }), this.notes)
      })), _resolve)
      })

      _result = await $Notes_Store.saveNote.bind($Notes_Store)(id, text)
      }
      catch (_error) {
        if (_error instanceof DoError) {} else {
            console.warn(`Unhandled error in sequence expression:`)
  ...
  setCurrentNote(text) {
    return $Notes_Store.setNote.bind($Notes_Store)(this.currentNote, text)
  }

  getCurrentNote() {
    return $Maybe.withDefault(``, $Array.at(this.currentNote, this.notes))
  }
  ...
  loadNotes() {
    return (async () => {
      ...
      try {
         await $Notes_Store.getNote.bind($Notes_Store)(1)

       await $Notes_Store.getNote.bind($Notes_Store)(2)

       await $Notes_Store.getNote.bind($Notes_Store)(3)

       await $Notes_Store.getNote.bind($Notes_Store)(4)

       await $Notes_Store.getNote.bind($Notes_Store)(5)

       await $Notes_Store.getNote.bind($Notes_Store)(6)

       await $Notes_Store.getNote.bind($Notes_Store)(7)

       await $Notes_Store.getNote.bind($Notes_Store)(8)

      _result = await $Notes_Store.getNote.bind($Notes_Store)(9)
      }
      catch (_error) {
        if (_error instanceof DoError) {} else {
            console.warn(`Unhandled error in sequence expression:`)
  ...
  getNote(id) {
    return (async () => {
      ...
      try {
        ...
        try {
          return await $Http.send($Http.get(`http://localhost:9900/note/` + $Number.toString(id)))
        } catch (_error) {
          let error = _error;
      ...
      let _1 = $Maybe.toResult(`Decode Error`, $Json.parse(response.body))
      ...
      let noteJson = _1.value

      let _2 = $$NoteObject.decode(noteJson)
      ...
      if (_2 instanceof Err) {
       ...
       _result = (async () => {
        ...
        try {
           await $Debug.log(`Could not decode!`)
      ...
      _result = await $Notes_Store.setNote.bind($Notes_Store)(id - 1, note.note)
      }
      catch (_error) {
        if (_error instanceof DoError) {} else {
            console.warn(`Unhandled error in sequence expression:`)
  ...
  saveNote(id, note) {
    return (async () => {
      ...
      try {
        let noteJson = await _encode(new $$NoteObject({
        note: note
      }))

      let response = await (async () => {
        try {
          return await $Http.send($Http.stringBody($Json.stringify(noteJson), $Http.header(`Content-Type`, `application/json`, $Http.put(`http://localhost:9900/note/` + $Number.toString(id + 1)))))
        } catch (_error) {
          let error = _error;
  ...
  quitNotePad() {
    return (async () => {
      ...
      try {
        let response = await (async () => {
        try {
          return await $Http.send($Http.put(`http://localhost:9900/quit`))
        } catch (_error) {
          let error = _error;
...
$Notes_Store.__displayName = `Notes.Store`

const $Style_Store = new (class extends Store {
    constructor() {
    super()
    this.state = {
        targetColors: [`#CC5A7B`, `#E47D98`, `#FF8CAA`, `#FFBBD0`, `#F1C0CC`, `#F9E2EA`, `#C2B1DB`, `#B09BD2`, `#8279AC`],
  ...
  get targetColors() { return this.state.targetColors }

  get appBackground() { return this.state.appBackground }

  get textColor() { return this.state.textColor }

  get editorBackground() { return this.state.editorBackground }

  get editorTheme() { return this.state.editorTheme }

  get shadowColor() { return this.state.shadowColor }

  get selectionBackgroundColor() { return this.state.selectionBackgroundColor }

  get selectionColor() { return this.state.selectionColor }

  get progName() { return this.state.progName }

  getTargetColors(id) {
    return $Maybe.withDefault(``, $Array.at(id, this.targetColors))
  }
})
$Style_Store.__displayName = `Style.Store`
...
class $NoteEditor extends Component {
  ...
  get themeFile() {
    return [`/np/codemirror/codemirror.css`, `/np/editorThemes/` + this.editorTheme + `.css`]
  }

  get lnumber () {
    if (this.props.lnumber != undefined) {
      return this.props.lnumber
    } else {
      return false
    }
  }

  get curID() { return this.state.curID }

  get currentNote () { return $Notes_Store.currentNote }

  getCurrentNote (...params) { return $Notes_Store.getCurrentNote(...params) }

  setCurrentNote (...params) { return $Notes_Store.setCurrentNote(...params) }

  get textColor () { return $Style_Store.textColor }

  get editorBackground () { return $Style_Store.editorBackground }

  get editorTheme () { return $Style_Store.editorTheme }

  componentWillUnmount () {
    $Notes_Store._unsubscribe(this);$Style_Store._unsubscribe(this)
  }

  componentDidMount () {
    $Notes_Store._subscribe(this);$Style_Store._subscribe(this)
  }
  ...
  handleChange(value) {
    return (async () => {
      ...
      try {
        _result = await (_compare(this.curID, this.currentNote) ? this.setCurrentNote.bind(this)(value) : (async () => {
        let _result = null;

        try {
           await this.setCurrentNote.bind(this)(value)

        _result = await new Promise((_resolve) => {
          this.setState(_update(this.state, new Record({
        curID: this.currentNote
        })), _resolve)
        })
        }
        catch (_error) {
  ...
  render() {
    return _createElement("div", {
      "id": `editorDiv`,
      className: `note-editor-editor`,
      style: {
        [`--note-editor-editor-background-color`]: this.editorBackground,
        [`--note-editor-editor-color`]: this.textColor
      }
    }, [_createElement($CodeMirror, { "onChange": this.handleChange.bind(this), "value": this.getCurrentNote.bind(this)(), "theme": this.editorTheme, "mode": `markdown`, "javascripts": [`/np/codemirror/codemirror.js`, `/np/codemirror/markdown.js`], "lineNumbers": this.lnumber, "styles": this.themeFile })])
  }
}
...
class $Header extends Component {
  get appBackground () { return $Style_Store.appBackground }

  componentWillUnmount () {
    $Style_Store._unsubscribe(this)
  }

  componentDidMount () {
    $Style_Store._subscribe(this)
  }

  render() {
    return _createElement("div", {
      className: `header-svgcontainer`,
      style: {
        [`--header-svgcontainer-color`]: this.appBackground
      }
    }, [_createElement("svg", {
      "width": `100%`,
      "height": `100%`,
...
class $NoteBar extends Component {
  getTargetColors (...params) { return $Style_Store.getTargetColors(...params) }
  ...
  render() {
    return _createElement("div", {
      className: `note-bar-notebarstyle`
    }, [_createElement($CircleTarget, { "id": 0, "color": this.getTargetColors.bind(this)(0) }), _createElement($CircleTarget, { "id": 1, "color": this.getTargetColors.bind(this)(1) }), _createElement($CircleTarget, { "id": 2, "color": this.getTargetColors.bind(this)(2) }), _createElement($CircleTarget, { "id": 3, "color": this.getTargetColors.bind(this)(3) }), _createElement($CircleTarget, { "id": 4, "color": this.getTargetColors.bind(this)(4) }), _createElement($CircleTarget, { "id": 5, "color": this.getTargetColors.bind(this)(5) }), _createElement($CircleTarget, { "id": 6, "color": this.getTargetColors.bind(this)(6) }), _createElement($CircleTarget, { "id": 7, "color": this.getTargetColors.bind(this)(7) }), _createElement($CircleTarget, { "id": 8, "color": this.getTargetColors.bind(this)(8) })])
  }
}
...
class $CircleTarget extends Component {
  get size() {
    return (_compare(this.currentNote, this.id) ? `38px` : `34px`)
  }

  get marginLeftRight() {
    return (_compare(this.currentNote, this.id) ? `3px` : `5px`)
  }
  ...
  setCurrentNoteId (...params) { return $Notes_Store.setCurrentNoteId(...params) }

  componentWillUnmount () {
    $Notes_Store._unsubscribe(this)
  }

  componentDidMount () {
    $Notes_Store._subscribe(this)
  }
  ...
  render() {
    return _createElement("div", {
      ...
      return (async () => {
        ...
        try {
          _result = await this.setCurrentNoteId.bind(this)(this.id)
        }
        catch (_error) {
          if (_error instanceof DoError) {} else {
              console.warn(`Unhandled error in sequence expression:`)
      ...
      className: `circle-target-base`,
      style: {
        [`--circle-target-base-height`]: this.size,
        [`--circle-target-base-width`]: this.size,
        [`--circle-target-base-border-radius`]: this.size,
        [`--circle-target-base-margin`]: `5px ` + this.marginLeftRight + ` 5px ` + this.marginLeftRight,
        [`--circle-target-base-background-color`]: this.color
      }
    })
  }
}

$CircleTarget.displayName = "CircleTarget"
...
class $Main extends Component {
  ...
  quitNotePad (...params) { return $Notes_Store.quitNotePad(...params) }

  componentWillUnmount () {
    $Style_Store._unsubscribe(this);$Notes_Store._unsubscribe(this)
  }

  componentDidMount () {
    $Style_Store._subscribe(this);$Notes_Store._subscribe(this)
  }
  ...
  render() {
    return _createElement("div", {
      ...
      "onKeyUp": (event => (((event) => {
      return (event.ctrlKey && (_compare(event.keyCode, 81)) ? this.quitNotePad.bind(this)() : $Utility.doNothing())
      }))(_normalizeEvent(event))),
      className: `main-outside-container`
    }, [_createElement($Header, {  }), _createElement("div", {
      className: `main-container-body`,
      style: {
        [`--main-container-body-background-color`]: this.appBackground
      }
    }, [_createElement($NoteEditor, {  }), _createElement($NoteBar, {  })])])
  }
}
...
_insertStyles(`
  .note-editor-editor {
    ...
    width: 480px;
    resize: none;
    background-color: var(--note-editor-editor-background-color);
    color: var(--note-editor-editor-color);
    font-size: 15px;
  ...
  .header-pathstyle {
    stroke: var(--header-pathstyle-stroke);
    stroke-width: 1px;
    fill: var(--header-pathstyle-fill);
  }
  ...
  .header-svgcontainer {
    ...
    width: 500px;
    height: 13px;
    color: var(--header-svgcontainer-color);
    margin: 0px;
    padding: 0px;
    -webkit-user-select: none;
    -moz-user-select: none;
  ...
  .circle-target-base {
    ...
    height: var(--circle-target-base-height);
    width: var(--circle-target-base-width);
    border-radius: var(--circle-target-base-border-radius);
    margin: var(--circle-target-base-margin);
    padding: 0px;
    background-color: var(--circle-target-base-background-color);
    cursor: pointer;
    -webkit-user-select: none;
    -moz-user-select: none;
    -ms-user-select: none;
  ...
  .main-container-body {
    ...
    height: 100%;
    width: 100%;
    background-color: var(--main-container-body-background-color);
